...
##1. Introduction
Distance Sampling is a process in which an area is surveyed so that the size of a population within an area can be estimated.  Distances from a set of transects to observed objects are recorded and these data are used to model a detection function.  From this the population in the whole area can be estimated.  

###1.1 Introduction to DSsim

DSsim is a tool which simulates this process.  DSsim splits the process into three stages.  First it generates a population.  DSsim then simulates the distance sampling survey.  This is done using a detection function and a survey design.  For the final stage DSsim analyses the data from the survey.

![Figure 1 - A simulation is composed of several other objects including the region and a survey design.  If you wish to change one of these components you will also have to remake the simulation afterwards to update the simulation.](SimulationDiagram.png)

When fitting a detection function the data is often truncated at some distance from the transect.  The number of detections will decrease at larger distances from the transect and observations can lead to poorer precision in the estimate of abundance by distorting the shape of the detection function.  It has been suggeste to truncate the data where the estimated probability of detection is 0.15 [@Buckland:2001vm] however this has been a topic of much discussion.

This vignette explores how truncation distances affect the accuracy and precision of the estimates of a population size.  This will be used to provide examples of how to use DSsim to make and run simulations as well as how to analyse the results.  Simulations will be run with truncation distances at different multiples of $\sigma$ as shown in Figure 2.  For a half normal detection function the suggested truncation distance is approximately 2$\sigma$ and for a hazard rate detection function with shape parameter 4 this is approximately 1.5$\sigma$. 

![Figure 2 - A half normal and hazard rate distributions with vertical lines showing the truncation distances used in this vignette](Rplots.png)

The rest of this vignette will guide you through the steps to create and run a series of simulations to investigate the effects of varying truncation distance.  For the advanced reader we also include sections describing how to save the data from a simulation in order to speed up subsequent simulations using the same population and on how to include model selection in the investigation.  In the results section we describe how truncation distance affects the accuracy and precision of the distance sampling abundance estimates.  Finally we conclude by discussing the implications of our findings for carrying out distance sampling surveys in practice.
...
```
Since these simulations take a long time to run we do not run them here.  However we have included the results of these simulations in the package so we will load them so that we can interperet them in this vignette.  We have also loaded some results from simulations where DSsim was allowed to choose between a half normal and a hazard rate key for the detection function using both the AIC and the BIC criteria as well as some where the true detection function had a hazard rate key:
```{r}
...
We use a switch statement to allow the user to specify the required information and then return the value from the appropriate slot in the simulation object:
...
```{r results="asis", fig.cap = "Table 2 - The results from the model selection simulations"}
table<-kable(simulation.data, 
             digits = 2, 
             col.names = c("$Trunc Dist$", "$n$", "$Mean \\hat{N}$", "$SE$", "$\\% Bias$", "$RMSE$"),
             align = c('c', 'c', 'c', 'c', 'c', 'c', 'c') )
...
```{r results="asis", fig.cap = "Table 3 - The results from the model selection simulations when the true detection function has a hazard rate key"}
...
```{r fig.cap = "Figure 9b - There is a clear downwards trend in RMSE as less data is truncated", fig.align="center", fig.width = 5, fig.height = 5}
plot(Trunc.Dist[1:6], RMSE.aic)
```

```{r fig.cap = "Figure 9c - There is a clear downwards trend in RMSE as less data is truncated", fig.align="center", fig.width = 5, fig.height = 5}
plot(Trunc.Dist[1:6], RMSE.bic)
```

```{r fig.cap = "Figure 9d - There is a clear downwards trend in RMSE as less data is truncated", fig.align="center", fig.width = 5, fig.height = 5}
plot(Trunc.Dist[1:6], RMSE.hr.aic)
```

```{r fig.cap = "Figure 9e - There is a clear downwards trend in RMSE as less data is truncated", fig.align="center", fig.width = 5, fig.height = 5}
...
```{r fig.cap = "Figure 11", fig.align="center", fig.width = 5, fig.height = 5}
library(ggplot2)
...
```{r fig.cap = "Figure 12", fig.align="center", fig.width = 5, fig.height = 5}
library(ggplot2)
...
```{r fig.cap = "Figure 13", fig.align="center", fig.width = 5, fig.height = 5}
library(ggplot2)
...
```{r fig.cap = "Figure 14", fig.align="center", fig.width = 5, fig.height = 5}
library(ggplot2)
