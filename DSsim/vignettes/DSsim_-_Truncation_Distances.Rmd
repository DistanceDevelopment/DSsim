...
```{r region, fig.cap="Figure 3", fig.align="center", fig.width = 7, fig.height = 5}
...
If the setup has been done right the simulation can now be run:
```{r first.sim, cache = TRUE, warning=FALSE, message=FALSE}
simtrunc125 <- run(simtrunc125, save.data=TRUE, data.path = "Results")
```
...
However this investigation involves running multiple simulations and spotting patterns from the summaries can be quite tedious.  Instead we will write some functions to extract the required information from the simulation objects.  It is not necessary to understand how these functions work, details are provided here for interested readers however the reader could just run the code and skip to the next section if they prefer.
...
Once the results have been extracted the package _xtable_ will be used to table the results:
```{r message=FALSE}
library(xtable)
```
...
table<-xtable(simulation.data, label = "Table", align = c('c', 'c', 'c', 'c', 'c', 'c', 'c') , digits = c(0, 0, 0, 2, 2, 2, 2))
colnames(table) <- c("$Trunc Dist$", "$n$", "$Mean \\hat{N}$", "$SE$", "$\\% Bias$", "$RMSE$")
