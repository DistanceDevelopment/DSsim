...
##1. Introduction
Distance Sampling is a process in which an area is surveyed so that the size of a population within the area can be estimated.  Distances from a set of transects to observed objects are recorded and these data are used to model a detectability function.  From this the population in the whole area can be estimated.  

###1.1 Introduction to DSsim

DSsim is a tool which simulates this process.  DSsim splits the process into three stages.  First it generates a population.  DSsim then simulates the distance sampling survey.  This is done using a detectability function and a survey design.  For the final stage DSsim analyses the data from the survey.
...
```{r analyses}
ddf.analyses <- make.ddf.analysis.list(dsmodel = list(~cds(key = "hn", formula = ~1)), method = "ds", truncation = 125)
```

###2.2.5.1 Model Selection
In the real world we do not know the shape of the detection function and must use model selection criteria to decide between models of different shape.  Here we use the AIC to decide between a hazard rate model and a half normal model:
```{r analyses2}
...
```{r results="asis", fig.cap = "Table 1"}
table<-kable(simulation.data, digits = 2, col.names = c("$Trunc Dist$", "$n$", "$Mean \\hat{N}$", "$SE$", "$\\% Bias$", "$RMSE$"), align = c('c', 'c', 'c', 'c', 'c', 'c', 'c') )
