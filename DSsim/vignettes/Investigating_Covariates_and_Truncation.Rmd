...
The next step in distance sampling is then to record the distances from each detected animal to the transect it was detected from and fit a detection function. From this function we can estimate how many objects were missed and hence the total number in the covered area. And example of a detection function is given in Figure 1. If our line transects have been placed at random within the study are then we would expect on average  
...
While these simulations focus on the the issue of truncation at the analysis stage, an example of using DSsim to compare survey designs can be found at <http://www.creem.st-and.ac.uk/DS.M&A/#survey-design-simulation-case-study>.
...
Here we load the DSsim library.
...
Determinine candidate truncation distances for the simulation with covariates in is slightly more complex due to the fact that we have a mixture of detection functions. 

```{r truncate, eval = TRUE, cache=TRUE, echo = FALSE, fig.width=4, fig.cap="histogram of data from covariate simulation."}
set.seed(2320)
temp <- sim.cov
...
for(i in seq(along = trunc.vals)){
  lines(x = rep(trunc.vals[i],2), y = c(0,trunc.y[i]), lwd = 2, col = 2)
}
...
for(tdist in seq(along= truncation)){
  # So we know how far through we are
  cat("\n Running for truncation = ", truncation[tdist], fill = T)
  # Update analysis truncation distance
  new.ddf.analyses <- make.ddf.analysis.list(dsmodel = list(~cds(key = "hn", formula = ~1),
                                                            ~cds(key = "hr", formula = ~1)), 
...
If our interest lies more in learning how individual level covariates affect detecability rather than just if they affect our estimates of density and abundance then we require a slightly different setup. DSsim does not store the detection function parameter estimates therefore we need to do this manually. As before we create our simulation but then we need to manually run the analyses and store the parameter estimates we are interested in.

```{r covsimulation, eval = FALSE}
sim.cov <- make.simulation(reps = 100, 
                       region.obj = region,
                       design.obj = design,
                       population.description.obj = pop.desc.cov,
                       detectability.obj = detect.cov,
...
sigma.est <- numeric(0)
male.param <- numeric(0)
N.est <- numeric(0)

for(i in 1:sim.cov@reps){
  ...
  sigma.est[i] <- ddf.results$par[1]
  male.param[i] <- ddf.results$par[2]
  N.est[i] <- dht.results$individuals$N$Estimate
}

oldpar <- par(mfrow = c(2,2))
hist(sigma.est, xlab = "Estimate of sigma", main = "Sigma")
...
hist(N.est, xlab = "Estimate of N", main = "N")
abline(v = sim.cov@population.description@N, col = 2, lwd = 2)
par(oldpar)
```

## 3. Results

As these simulations take a substantial amount of time to run we have saved the results summaries into this package. They can be loaded and viewed as follows:

```{r loadresults, eval = FALSE}

sim.summaries <- data("trunc.hn.summaries.Robj")
summary.t200 <- sim.summaries[[1]]
summary.t400 <- sim.summaries[[2]]
summary.t600 <- sim.summaries[[3]]

sim.summaries <- data("trunc.cov.summaries.Robj")
summary.cov.t200 <- sim.summaries[[1]]
summary.cov.t400 <- sim.summaries[[2]]
summary.cov.t600 <- sim.summaries[[3]]

# We can then simply type the name of the summary that we wish to view
summary.t200
