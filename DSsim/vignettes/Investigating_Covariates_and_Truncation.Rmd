...
![Figure 2: Illustrates the simulation process. Blue rectangles indicate information supplied by the user. Green rectangles are objects created by DSsim in the simulation process. Orange diamonds indicate the processes carried out by DSsim.](../../vignette/images/SimulationDiagram.png)
...
As detailed in section 1.1 a simulation comprises of a number of components. DSsim is designed so that each of these components is defined individually before they are grouped together into a simulation. This helps keep the process clear and also allows reuse of simulation components between different simulations. Each of the functions to create a simulation component or simulation takes the form *make.<component>*.
...
The simulation is created by grouping all these components together. It is then a good idea to check that everything is as you indended. The function *check.sim.setup* provides a number of plots to help you do this. 
...
For the simulation with covatiates we can re-use the region, design and analysis components and then add in the new population description and detecability to include the sex covariate.
...
The above simulations concentrate on the question of how accurately and precisely we can estimate the abundance and density of a population. However, we may also be interested in learning how individual level covariates affect detecability. To do this we require a slightly different setup. DSsim does not currently store the detection function parameter estimates therefore we need to do this manually, however DSsim does provide functions so that doing this is fairly straight forward. As before we create our simulation but then we need to get DSsim to give us the survey data so that we can run the analyses and obtain the parameter estimates. Please note that the extraction of the parameter estimates from the ddf model is specific to this model, if you are adapting this code you will need to check the ddf documentation to understand the parameters for different models.
...
As these simulations take a substantial amount of time to run we have saved the results summaries into this package as data. They can be loaded as follows:
...
While the estimates of abundance are not greatly affected by truncation distance for these simulations, the same cannot be said for the parameter estimates. Figure 13, suggests that parameter estimation is most accurate and reliable at maximum truncation distance. The unstable parameter estimates with small truncation distance could explain the small bias in abundance estimates for these truncation distances. It is hoped that in practise this strange behavour would be identified and these estimates rejected based on model selection criteria.
...
```{r makeplots, eval= TRUE, echo = FALSE, fig.width=7.2, fig.cap="Figure 13: Histograms of the parameter estimates for sigma and sex.male for three of the five truncation distances investigated."}
...
* Larger truncation distances or no truncation is preferable when trying to obtain the parameters for the covariates which affect detectability.
